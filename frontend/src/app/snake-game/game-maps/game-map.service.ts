...
export class GameMapService {
  ...
  fetchGameMaps(isPublic: boolean, isOfficial: boolean) {
  ...
  fetchGameMap(gameMapType: GameMapType, gameMapId: string) {
    const httpUrl = environment.apiUrl + `game-maps/${gameMapId}`;
  ...
  editUserScore(mapId: string, userScore: UserScore) {
    ...
    const httpUrl = environment.apiUrl + `game-maps/user-score/edit/${mapId}`;

    return this.http.put<any>(httpUrl, userScore).subscribe();
  }
