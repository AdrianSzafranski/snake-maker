...
export class SnakeModel {
    ...
    constructor(coords: Coordinate[], direction: string, color: string) {
    ...
    getDestination(gameMapHorizontalLenInElements: number, gameMapVerticalLenInElements: number) {
        ...
        switch (this.direction) {
            case 'up':
                newCoord.y = (newCoord.y - 1 >= 0)
                    ? newCoord.y - 1
                    : gameMapVerticalLenInElements - 1;
                break;
            case 'right':
                newCoord.x = (newCoord.x + 1 < gameMapHorizontalLenInElements)
                    ? newCoord.x + 1
                    : 0;
                break;
            case 'down':
                newCoord.y = (newCoord.y + 1 < gameMapVerticalLenInElements)
                    ? newCoord.y + 1
                    : 0;
                break;
            case 'left':
                newCoord.x = (newCoord.x - 1 >= 0)
                    ? newCoord.x - 1
                    : gameMapHorizontalLenInElements - 1;
                break;
        }
    ...
    eat(foodCoordinate: Coordinate, elongationNumber: number) {
